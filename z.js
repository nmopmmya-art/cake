...
document.querySelector(".flowers-soon").style.display = "none";
 document.querySelector(".pancake-banner").style.display = "none";
    document.querySelector(".pancake-products").style.display = "none";
        // اظهار المنتج المضغوط فقط
        element.style.display = "flex";

        // اخفاء النص القصير
        let text = element.querySelector(".wsf");
        text.style.display = "none";

        // اظهار التفاصيل
        let hidee = element.querySelector(".hidee");
        hidee.style.display = "block";

        // 🔴 نخزن hidee داخل العنصر نفسه عشان نستخدمه بالرجوع
        element.currentHidee = hidee;
        element.currentText = text;
...
document.querySelectorAll(".btn-back").forEach(btn => {
    ...
    btn.addEventListener("click", function(e){
        ...
        if(isBirth){
            birthItems.forEach(el => el.style.display = "block");
        }

        if(isVf){
            vfItems.forEach(el => el.style.display = "block");
        }

        if(isEvent){
            eventItems.forEach(el => el.style.display = "block");
        }

        if(isForr){
            forrItems.forEach(el => el.style.display = "block");
        }
